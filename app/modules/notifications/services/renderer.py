...
def build_context(
    ...
    **extra,
    ...
    f_amount_due = f"{amount_due:,.2f}"
    f_previous_arrears = f"{previous_arrears:,.2f}"
    f_penalty = f"{penalty:,.2f}"

    ctx = {
        "payer_name":       payer_name,
        "amount_due":       f_amount_due,
    ...
    if is_rollover:
        ctx["rollover_block_txt"] = (
            f"\n--- ROLLOVER DETAILS ---\n"
            f"Previous Unpaid Balance: {currency} {f_previous_arrears}\n"
            f"Late Payment Penalty: {currency} {f_penalty}\n"
            f"------------------------\n"
        )
        ctx["rollover_block_html"] = f"""
        <div style="background: #fffbeb; border: 1px solid #fde68a; border-radius: 12px; padding: 16px; margin: 16px 0;">
            <div style="color: #92400e; font-weight: bold; margin-bottom: 8px;">Rollover & Penalty Applied</div>
            <div style="display: flex; justify-content: space-between; margin: 4px 0; color: #b45309;"><span>Previous Balance:</span> <b>{currency} {f_previous_arrears}</b></div>
            <div style="display: flex; justify-content: space-between; margin: 4px 0; color: #b45309;"><span>Penalty:</span> <b>{currency} {f_penalty}</b></div>
            <div style="font-size: 13px; color: #d97706; margin-top: 8px;">These amounts are included in your current invoice total of {currency} {f_amount_due}.</div>
        </div>
        """
    else:
        ctx["rollover_block_txt"] = ""
